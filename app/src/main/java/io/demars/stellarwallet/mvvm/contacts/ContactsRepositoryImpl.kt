package io.demars.stellarwallet.mvvm.contacts
...
import io.demars.stellarwallet.models.StellarContact
...
object ContactsRepositoryImpl : ContactsRepository {
  ...
  private fun getStellarContacts(): List<StellarContact> {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M &&
      appContext.checkSelfPermission(READ_CONTACTS) != PERMISSION_GRANTED) return list
  ...
  private fun getContactsList(): Cursor? {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M &&
      appContext.checkSelfPermission(READ_CONTACTS) != PERMISSION_GRANTED) return null
